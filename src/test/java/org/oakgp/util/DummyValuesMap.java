...
public class DummyValuesMap<K, V> {
   ...
   public V next(K key) {
      ...
      if (values == null) {
         ...
         throw new IllegalArgumentException("No remaning values found for key: " + key + " of type: " + key.getClass().getName());
      } else {
         return values.next();
      }
   }
   ...
   public static class Builder<K, V> {
      ...
      public Builder<K, V> put(K key, DummyValuesQueue<V> values) {
         if (map.containsKey(key)) {
            throw new IllegalStateException("Vakues already exist for key: " + key);
         } else {
            map.put(key, values);
         }
         return this;
